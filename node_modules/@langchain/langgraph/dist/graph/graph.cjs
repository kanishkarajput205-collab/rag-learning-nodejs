...
var Branch = class {
	...
	constructor(options) {
		...
		else this.path = (0, _langchain_core_runnables._coerceToRunnable)(options.path);
	...
	async _route(input, config, writer, reader) {
		...
		if (destinations.filter(require_constants._isSend).some((packet) => packet.node === "__end__")) throw new require_errors.InvalidUpdateError("Cannot send a packet to the END node");
...
var Graph = class {
	...
	addConditionalEdges(source, path, pathMap) {
		...
		this.warnIfCompiled("Adding an edge to a graph that has already been compiled. This will not be reflected in the compiled graph.");
		if (!_langchain_core_runnables.Runnable.isRunnable(options.path)) options.path = (0, _langchain_core_runnables._coerceToRunnable)(options.path);
...
var CompiledGraph = class extends require_pregel_index.Pregel {
	...
	attachBranch(start, name, branch) {
		if (start === "__start__" && !this.nodes["__start__"]) this.nodes[require_constants.START] = require_pregel_index.Channel.subscribeTo(require_constants.START, { tags: [require_constants.TAG_HIDDEN] });
		this.nodes[start].pipe(branch.run((dests) => {
			return new require_write.ChannelWrite(dests.map((dest) => {
				...
				return {
					channel: dest === "__end__" ? require_constants.END : `branch:${start}:${name}:${dest}`,
					value: require_write.PASSTHROUGH
				};
			}), [require_constants.TAG_HIDDEN]);
		}));
